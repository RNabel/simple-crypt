...
def encrypt(password, data, cipher_storage=None):
    ...
    if cipher_storage:
        ...
        _hmac_key, _cipher_key = _expand_keys(password, _salt, EXPANSION_COUNT[LATEST])  # Generate cipher_key and HMAC.

    # Creates random new counter to fulfil uniqueness property when encrypting block-by-block.
    ...
    # Create HMAC across data.
    hmac = _hmac(_hmac_key, HEADER[LATEST] + _salt + encrypted)
...
def decrypt(password, data, cipher_storage=None):
    ...
    hmac2 = _hmac(_hmac_key, data[:-HASH.digest_size])

    _assert_hmac(_hmac_key, hmac, hmac2)
    counter = Counter.new(HALF_BLOCK, prefix=_salt[:HALF_BLOCK // 8])
    cipher = AES.new(_cipher_key, AES.MODE_CTR, counter=counter)
